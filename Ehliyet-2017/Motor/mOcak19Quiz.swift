...
class mOcak19Quiz: UIViewController,  GADInterstitialDelegate   {
    
    var fullScreenads :GADInterstitial!
    
    let allQuestions = mOcak19Bank()
    ...
    func alert(){
        ...
        let ok = UIAlertAction(title: "TAMAM", style: UIAlertAction.Style.default, handler: {(action) -> Void in
            ...
            UserDefaults.standard.set(self.correct, forKey: "vmCOcak19")
            UserDefaults.standard.synchronize()
            UserDefaults.standard.set(self.incorrect, forKey: "vmIOcak19")
            UserDefaults.standard.synchronize()
            self.resultSave()
            self.performSegue(withIdentifier: "mOcak19", sender: self)
        })
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if UserDefaults.standard.object(forKey: "vmCOcak19") != nil {
            self.correct = UserDefaults.standard.object(forKey: "vmCOcak19") as! Int
        }
        if UserDefaults.standard.object(forKey: "vmIOcak19") != nil {
            self.incorrect = UserDefaults.standard.object(forKey: "vmIOcak19") as! Int
        }
        
        let mOcak19 = segue.destination as! motor17
        mOcak19.correctOcak19 = correct
        mOcak19.incorrectOcak19 = incorrect
        mOcak19.puanOcak19 = String(correct*2)
    }
    ...
    func resultSave() {
        ...
        result.dersadi = "Motor Ocak 2019 E-Sınav"
        result.tarih = tarihh
