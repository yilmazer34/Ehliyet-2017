...
class mMarch19Quiz: UIViewController,  GADInterstitialDelegate   {
    
    var fullScreenads :GADInterstitial!
    
    let allQuestions = mMarch19Bank()
    ...
    func alert(){
        ...
        let ok = UIAlertAction(title: "TAMAM", style: UIAlertAction.Style.default, handler: {(action) -> Void in
            ...
            UserDefaults.standard.set(self.correct, forKey: "vmCMarch19")
            UserDefaults.standard.synchronize()
            UserDefaults.standard.set(self.incorrect, forKey: "vmIMarch19")
            UserDefaults.standard.synchronize()
            self.resultSave()
            self.performSegue(withIdentifier: "mMarch19", sender: self)
        })
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if UserDefaults.standard.object(forKey: "vmCMarch19") != nil {
            self.correct = UserDefaults.standard.object(forKey: "vmCMarch19") as! Int
        }
        if UserDefaults.standard.object(forKey: "vmIMarch19") != nil {
            self.incorrect = UserDefaults.standard.object(forKey: "vmIMarch19") as! Int
        }
        
        let mMarch19 = segue.destination as! motor17
        mMarch19.correctMarch19 = correct
        mMarch19.incorrectMarch19 = incorrect
        mMarch19.puanMarch19 = String(correct*2)
    }
    ...
    func resultSave() {
        ...
        result.dersadi = "Motor Mart 2019 E-Sınav"
        result.tarih = tarihh
