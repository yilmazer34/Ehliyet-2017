...
class Ocak19Quiz: UIViewController,  GADInterstitialDelegate   {
    
    var fullScreenads :GADInterstitial!
    
    let allQuestions = Ocak19Bank()
    ...
    func alert(){
        ...
        let ok = UIAlertAction(title: "TAMAM", style: UIAlertAction.Style.default, handler: {(action) -> Void in
            ...
            UserDefaults.standard.set(self.correct, forKey: "vCOcak19")
            UserDefaults.standard.synchronize()
            UserDefaults.standard.set(self.incorrect, forKey: "vIOcak19")
            UserDefaults.standard.synchronize()
            self.resultSave()
            self.performSegue(withIdentifier: "Ocak19", sender: self)
        })
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if UserDefaults.standard.object(forKey: "vCOcak19") != nil {
            self.correct = UserDefaults.standard.object(forKey: "vCOcak19") as! Int
        }
        if UserDefaults.standard.object(forKey: "vIOcak19") != nil {
            self.incorrect = UserDefaults.standard.object(forKey: "vIOcak19") as! Int
        }

        let Ocak19 = segue.destination as! List2019
    ...
    func resultSave() {
        ...
        result.dersadi = "Ocak 2019 E-Sınav"
        result.tarih = tarihh
